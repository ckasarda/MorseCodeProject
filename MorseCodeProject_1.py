...
## This code allows capital and small letters
    # If the letter is capitalized, during the encode, one '*' is added in front off the morse code
    # during the decode, the program see if the first symbol of the morse code is a '*',
...
## The program works even if one symbol that is not in the is not in the "MorseTable.txt" file
    # even if this symbol is being used in the file with english letters
...
def English2Morse ():
    ...
    if excp:
        if excp2:
            ...
            shell_connect.write('\nEnglish to Morse partially sucess, some symbols was not found and ignored.','KEYWORD')
...
def Morse2English():
    ...
    if excp:
        if excp2:
            ...
            shell_connect.write('\nMorse to English partially sucess, some morse code was not found and ignored.','KEYWORD')
        print('\nThe file {} was created.'.format(filename[:-4]+'_unhidden.txt'))
    else:
        shell_connect.write('\nMorse to English fail. {}\n'.format(excpwhy),'COMMENT')
